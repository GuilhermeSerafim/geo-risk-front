...
type ScrollHeroProps = {
  sections?: ScrollHeroSection[];
  colorPalette?: ScrollHeroPalette;
  logo?: string;
  menuItems?: string[];
};

const defaultSections: ScrollHeroSection[] = [
...
const defaultColorPalette: ScrollHeroPalette = {
  primary: '#6366f1',
  secondary: '#8b5cf6',
  tertiary: '#ec4899',
  accent: '#06ffa5',
...
const defaultMenuItems = ['Work', 'About', 'Services', 'Contact'];
...
const ScrollHero = ({
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      cinePass = new ShaderPass(cinematicPostShader);
  ...
  useEffect(() => {
    ...
    let vel = 0;
    let velTimeout: ReturnType<typeof window.setTimeout> | undefined;
    ...
    const rootTrigger = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        if (velTimeout !== undefined) {
          window.clearTimeout(velTimeout);
        }
